...
public class SearchController {
    ...
    @GetMapping("/download")
    public ResponseEntity<InputStreamResource> download(@Valid FileRequest request, HttpServletResponse httpServletResponse)
            throws IOException {
        FileResponse response = new FileResponse();
        searchService.download(request, response);
        if (ConstantApi.CODE.SUCCESS.getCode().equals(response.getCode())) {
            FileSystemResource fileSystemResource = response.getFileSystemResource();
